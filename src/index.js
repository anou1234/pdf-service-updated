...
let app = express();
app.use(cors()); // Allow cross-origin requests (e.g. from localhost:3000 → localhost:8082)
...
// Config URLs can be provided as comma-separated lists. If not provided, leave empty.
let dataConfigUrls = (envVariables.DATA_CONFIG_URLS || "").trim();
let formatConfigUrls = (envVariables.FORMAT_CONFIG_URLS || "").trim();
...
let topicKeyMap = {};
var topic = [];
// Avoid split on undefined; count entries only if provided
var datafileLength = dataConfigUrls
  ? dataConfigUrls.split(",").filter(Boolean).length
  : 0;
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!signatureId || !pkcs7Signature) {
      return res.status(400).json({
        ResponseInfo: requestInfo,
        message: "signatureId and pkcs7Signature are required",
      });
    }
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      let data = await getDefaulterPdfRecordsDetails(
        uuid,
        offset,
        limit,
        jobId,
...
var i = 0;
dataConfigUrls &&
  dataConfigUrls.split(",").map((rawItem) => {
    let item = (rawItem || "").trim();
    // Determine if this is a local file path (supports file://, file:/// and raw Windows paths)
    let filePath = null;
    if (/^file:\/\//i.test(item)) {
      filePath = item.replace(/^file:\/\//i, ""); // strip both file:// and file:///
    } else if (/^[a-zA-Z]:[\\\/]/.test(item) || item.startsWith("\\\\")) {
      filePath = item; // Windows absolute path or UNC path
    }

    if (filePath) {
      const normalized = path.normalize(filePath);
      fs.readFile(normalized, "utf8", function (err, data) {
        try {
          if (err) {
            logger.error(
              "error when reading file for dataconfig: file:///" + normalized,
            );
            ...
            i++;
            if (i == datafileLength && envVariables.KAFKA_ENABLED) {
              topic.push(envVariables.KAFKA_RECEIVE_CREATE_JOB_TOPIC);
              listenConsumer(topic);
            }
            logger.info("loaded dataconfig: file:///" + normalized);
          }
        } catch (error) {
          logger.error("error in loading dataconfig: file:///" + normalized);
...
formatConfigUrls &&
  formatConfigUrls.split(",").map((rawItem) => {
    let item = (rawItem || "").trim();
    // Determine if this is a local file path (supports file://, file:/// and raw Windows paths)
    let filePath = null;
    if (/^file:\/\//i.test(item)) {
      ...
      fs.readFile(normalized, "utf8", function (err, data) {
        try {
          if (err) {
            logger.error(err.stack);
            logger.error(
              "error when reading file for formatconfig: file:///" + normalized,
            );
          } else {
            data = JSON.parse(data);
            formatConfigMap[data.key] = data.config;
            logger.info("loaded formatconfig: file:///" + normalized);
          }
        } catch (error) {
          logger.error("error in loading formatconfig: file:///" + normalized);
          logger.error(error.stack);
        }
      });
    } else {
...
if (envVariables.KAFKA_ENABLED) {
  ...
  logger.warn(
    "Kafka is disabled (KAFKA_ENABLED=false). Skipping consumer startup.",
  );
}
...
export const createNoSave = async (
  ...
  try {
    ...
    logger.info(
      "received createnosave request on key: " +
        key +
        " for job id:" +
        bulkPdfJobId +
        " totalPdfRecords: " +
...
const generateQRCodes = async (
  ...
  for (var i = 0, len = qrcodeMappings.length; i < len; i++) {
    ...
    else qrtext = mustache.render(urlQR, variableTovalueMap);
