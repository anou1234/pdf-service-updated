...
export const insertStoreIds = (
  ...
  producer.send(payloads, function (err, data) {
    if (err) {
      ...
      if (envVariables.KAFKA_STRICT) {
        // Preserve original behavior when strict mode is enabled
        errorCallback({
          message: `error while publishing to kafka: ${err.message}`
        });
      } else {
        // Graceful degradation: log and proceed as success so PDF generation doesn't fail
        logger.warn("Kafka publish failed, but continuing (KAFKA_STRICT=false)");
        successCallback({
          message: "Success",
          jobid: jobid,
          ...
          moduleName,
          kafkaStatus: "failed",
          kafkaError: err && err.message ? err.message : undefined
        });
      }
    } else {
      ...
      (async () => {
        ...
        try {
          ...
          producer.send(errorPlayloads, function (err, data) {
            if (err) {
              // Best-effort logging only for error topic; don't impact main flow
              logger.error(err.stack || err);
              logger.warn("Failed to publish error to Kafka; ignoring as non-fatal");
            }
          });
        }
...
export async function sendNoitification(filestoreid, mobileNumber, tenantId) {
  ...
  producer.send(payloads, function (err, data) {
    if (err) {
      // Notification failures shouldn't break main flow; just log
      logger.error(err.stack || err);
      logger.warn("Failed to publish notification to Kafka; ignoring as non-fatal");
    } else {
      logger.info("Notification published to Kafka successfully");
    }
  });
}
